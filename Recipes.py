...
class Recipes:
    ...
    def create_recipes_frame(self):
        ...
        self._headline=CTkLabel(master=self._recipes_window, text="Recipes", font=('Calibri', 50))
        #self._headline.place(x=410, y=0)
        self._headline.pack(pady=2,padx=10,side="top")
        self._btn_back = CTkButton(self._recipes_window, text="Back", height=30, width=80, text_color="white", hover_color="#4185D0", font=("Calibri", 17), fg_color="#C850C0",command=self.on_click_back)
        self._btn_back.pack(side="top", pady=2,padx=10)

    def on_click_back(self):
        self.on_click_destroy_specific_recipe()
        self._recipes_window.pack_forget()
    ...
    def on_click_cook(self,data):
        if not self._specific_recipe_frame:
            self._specific_recipe_frame = CTkFrame(master=self._recipes_window, width=700, height=350, corner_radius=20)
        headline = CTkLabel(master=self._specific_recipe_frame, text=data['name'], font=('Calibri', 25, "bold"))
        ...
        recipe_steps=CTkLabel(master=self._specific_recipe_frame,text=data['data'],anchor='w', justify="left",  wraplength=650,font=('Calibri', 18))
        recipe_steps.place(x=5,y=85)
        headline.place(x=5, y=0)
        description.place(x=5, y=30)
        btn_back.place(x=600,y=0)
